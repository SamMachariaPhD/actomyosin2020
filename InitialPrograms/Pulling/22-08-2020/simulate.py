...
Dt = 5e-7
max_comp_simulations = 16

files = ['MotilityAssayActin2MotorsParameters_v6.f90', 'MotilityAssayConfinements_v2.f90', \
...
param_file = "MotilityAssayActin2MotorsParameters_v6.f90"
...
while simulation_runs > 0:
    ...
    species_ratio, beads_number, ATP_value, MD_value, seed, simul_tym_sec, ExtForceMag = map(float,read_params[simulations_counter].split(',')) #dynamic parameters based on .txt file
    params.close()
    #species_ratio, beads_number, ATP_value, MD_value = map(float,input().split(',')) #Bad. I have to enter each param.
    new_dir = 'R'+str(species_ratio)+'B'+str(int(float(beads_number)))+'ATP'+str(int(float(ATP_value)))+'MD'+str(int(float(MD_value)))+'S'+str(int(float(seed)))+'T'+str(int(float(simul_tym_sec)))+'xF'+str(int(float(ExtForceMag))) #dynamic folder name
    ...
    for i, line in enumerate(fileinput.input(param_file, inplace=1)):
        sys.stdout.write(line.replace('ExtForceMag = 20.0', 'ExtForceMag = '+str(float(ExtForceMag)) ))
    #print ("\n=> Param. file in %s successfully updated." % new_dir)
    #print ("\n=> Simulation program started!: %s" % new_dir)
    subprocess.call("ifort mt.f90 MotilityAssayActin2MotorsParameters_v6.f90 MotilityAssayConfinements_v2.f90 MotilityAssaySubstrateDeformation_v2.f90 MotilityAssayForceForceFunctions_v3.f90 MotilityAssayActin2MotorsMain_v9.f90", shell=True)
