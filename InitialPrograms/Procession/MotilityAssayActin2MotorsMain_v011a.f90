PROGRAM BEADRODPOLYMER
    ...
    !---Forced Detachment of Motors---------------------------------------
    
	DO Ip=1, NumParticles
		CALL MotorForcedDetachment(F_Motor_X(Ip), F_Motor_Y(Ip), F_Motor_Z(Ip), ContactState(:,Ip), Release_ADP(Ip), Elongation(Ip), MotorSpecies(Ip))
	END DO	
...
!	DO Ip=1, NumParticles
!		CALL MotorForcedDetachment(F_Motor_X(Ip), F_Motor_Y(Ip), F_Motor_Z(Ip), ContactState(:,Ip), Release_ADP(Ip), Elongation(Ip), MotorSpecies(Ip))			!v9
!	END DO
	...
	SUBROUTINE MotorForcedDetachment(F_Motor_X_Ip, F_Motor_Y_Ip, F_Motor_Z_Ip, ContactState_Ip, Release_ADP_Ip, Elongation_Ip, MotorSpecies_Ip)
        ...
        SELECT CASE(MotorSpecies_Ip)

        CASE (1)

            IF (k*Elongation_Ip >= F_Motor_Detach1) THEN
	
                F_Motor_X_Ip = 0.0_DP
                F_Motor_Y_Ip = 0.0_DP
                F_Motor_Z_Ip = 0.0_DP
        
                Scan_Contact_Filament1: DO II=1, NumMol
        ...
        CASE (2)
            ...
            IF (k*Elongation_Ip >= F_Motor_Detach2) THEN

                F_Motor_X_Ip = 0.0_DP
                F_Motor_Y_Ip = 0.0_DP
                F_Motor_Z_Ip = 0.0_DP
        
                Scan_Contact_Filament2: DO II=1, NumMol
                    IF (ContactState_Ip(II) >= 1.0_DP) THEN
