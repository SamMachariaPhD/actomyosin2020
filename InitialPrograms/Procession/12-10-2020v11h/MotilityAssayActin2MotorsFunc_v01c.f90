...
!---Initial Motor Location----------------------------------------------------------------
	...
	SUBROUTINE InitialMotorLocations(ActiveMotorIdxOffset, ActiveMotorIdxEnd, IniAngle, X, Y, XM, YM, ZM, ContactState, AddedMotorNum, AreaCounter, AreaOriginX, AreaOriginY, AreaOriginUx, AreaOriginUy, MotorType)
		...
		DO IM=1, INT(Motor_Density*HorizontalLength*VerticalLength, Range15)
		...
		ActiveMotorIdxEnd = ActiveMotorIdxEnd + INT(Motor_Density*HorizontalLength*VerticalLength, Range15)
		AddedMotorNum(AreaCounter) = INT(Motor_Density*HorizontalLength*VerticalLength, Range15)
	...
	SUBROUTINE CheckMotorFilamentProximity(ActiveMotorIdxOffset, ActiveMotorIdxEnd, X, Y, Z, XM, YM, ZM, ContactState, TempContact)
	
		USE PARAMETERS, ONLY : DP, MaxNumMotors, NumFilament, NumBeads, BondLength, Stepsize
	...
	!!!---Assuming that NumFilament = 1
	
	
	!$omp parallel
	!$omp do private(JJ,Intercept,SqDistance)
		ScanningMotors: DO IM=ActiveMotorIdxOffset + 1, ActiveMotorIdxEnd
	
			IF (ContactState(1, IM) >= 1.0_DP) CYCLE ScanningMotors
			...
			DO JJ=1, NumBeads-1
				...
				Intercept = (XM(IM) - X(JJ,1))*(X(JJ+1,1) - X(JJ,1)) + &
							(YM(IM) - Y(JJ,1))*(Y(JJ+1,1) - Y(JJ,1)) + &
							(ZM(IM) - Z(JJ,1))*(Z(JJ+1,1) - Z(JJ,1))
				Intercept = Intercept/BondLength
		
				IF ((Intercept > BondLength) .OR. (Intercept < 0.0_DP)) CYCLE
	
				SqDistance = (XM(IM) - X(JJ,1))*(XM(IM) - X(JJ,1)) + &
							(YM(IM) - Y(JJ,1))*(YM(IM) - Y(JJ,1)) + &
							(ZM(IM) - Z(JJ,1))*(ZM(IM) - Z(JJ,1)) - Intercept**2
	
				IF (SqDistance <= CaptureRadius**2) THEN
					TempContact(1, IM) = DBLE(JJ) + Intercept/BondLength
				END IF
	
			END DO
	
	
		END DO ScanningMotors
	!$omp end do
	!$omp end parallel
	...
	SUBROUTINE MotorBinding(ActiveMotorIdxOffset, ActiveMotorIdxEnd, ContactState, TempContact)
		...
		REAL(KIND = DP), DIMENSION(MaxNumMotors) :: URArray
		...
		Loop_Motor_ContactCheck: DO IM=ActiveMotorIdxOffset+1, ActiveMotorIdxEnd
	
			Loop_Filament_ContactCheck: DO II=1, NumFilament
	
				IF ((TempContact(II, IM) >= 1.0_DP ) .AND. (INT(ContactState(II, IM)) == 0)) THEN
	
					IF(URArray(IM) > k_a*dt)THEN  
								TempContact(II, IM) = 0.0_DP
					END IF
	...
	SUBROUTINE MotorMove(ContactState_IM, TempContact_IM)
	...
	SUBROUTINE MotorForcedDetachment(F_Motor_X_IM, F_Motor_Y_IM, F_Motor_Z_IM, ContactState_IM, Release_ADP_IM, Elongation_IM, MotorType_IM)
