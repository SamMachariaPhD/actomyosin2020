PROGRAM MAIN
	...
	TempContact = 0.0_DP
	CALL CheckMotorFilamentProximity(ActiveMotorIdxOffset, ActiveMotorIdxEnd, X, Y, Z, XM, YM, ZM, ContactState, TempContact)
...
!---Forced Detachment of Motors---------------------------------------
	...
	DO IM=ActiveMotorIdxOffset+1, ActiveMotorIdxEnd
		CALL MotorForcedDetachment(F_Motor_X(IM), F_Motor_Y(IM), F_Motor_Z(IM), ContactState(:,IM), Release_ADP(IM), Elongation(IM), MotorType(IM))
	END DO	
    
    
	!---State Conversion of Bound Motors--------------------------------------------
	...
	TempContact = 0.0_DP
	CALL CheckMotorFilamentProximity(ActiveMotorIdxOffset, ActiveMotorIdxEnd, X, Y, Z, XM, YM, ZM, ContactState, TempContact)
	CALL MotorBinding(ActiveMotorIdxOffset, ActiveMotorIdxEnd, ContactState, TempContact)
	
	DO IM=ActiveMotorIdxOffset+1, ActiveMotorIdxEnd
		IF (MotorType(IM) == 1) THEN
			CALL MotorMove(ContactState(:,IM), TempContact(:,IM))
